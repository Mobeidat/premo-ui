...
<script>
  ...
  export default {
    ...
    props: {
      ...
      id: { type: String, default: 'PremoPhoneNumberInput' },
    ...
    async mounted () {
      try {
        ...
        if (this.defaultCountryCode && this.fetchCountry)
          throw new Error(`PremoPhoneNumberInput: Do not use 'fetch-country' and 'default-country-code' options in the same time`)
        if (this.defaultCountryCode && this.noUseBrowserLocale)
          throw new Error(`PremoPhoneNumberInput: If you use a 'default-country-code', do not use 'no-use-browser-locale' options`)
